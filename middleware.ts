...
function resolvePreferredLocale(request: NextRequest): AutoLocaleCode | "skip" {
  ...
  // Manual LanguageSwitcher preference ALWAYS wins — never override with Accept-Language.
  if (raw === "ko" || raw === "en") return raw;
  if (raw === "ja") return "skip"; // cookie wins; no /ja auto-routing in this change
...
export const config = {
  matcher: [
    /*
     * Match all paths except Next internals (also guarded in shouldSkip).
     */
